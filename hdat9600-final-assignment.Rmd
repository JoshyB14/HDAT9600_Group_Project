...
summary(cox.WBC_diff)  # significant

# fitting a simple Cox model with SaO2_min using coxph
cox.SaO2_min <- coxph( Surv(Days, Status) ~ SaO2_min, data=icu_patients_df1)
summary(cox.SaO2_min)

# fitting a simple Cox model with SaO2_diff using coxph
cox.SaO2_diff <- coxph( Surv(Days, Status) ~ SaO2_diff, data=icu_patients_df1)
summary(cox.SaO2_diff)

# fitting a simple Cox model with MAP_max using coxph
cox.MAP_max <- coxph( Surv(Days, Status) ~ MAP_max, data=icu_patients_df1)
summary(cox.MAP_max)

# fitting a simple Cox model with MAP_diff using coxph
cox.MAP_diff <- coxph( Surv(Days, Status) ~ MAP_diff, data=icu_patients_df1)
summary(cox.MAP_diff)  # significant

# fitting a simple Cox model with MAP_min using coxph
cox.MAP_min <- coxph( Surv(Days, Status) ~ MAP_min, data=icu_patients_df1)
summary(cox.MAP_min)    # significant
...
# fitting a simple Cox model with Albumin_max using coxph
cox.Albumin_max <- coxph( Surv(Days, Status) ~ Albumin_max, data=icu_patients_df1)
summary(cox.Albumin_max) # significant

# fitting a simple Cox model with Albumin_diff using coxph
cox.Albumin_diff <- coxph( Surv(Days, Status) ~ Albumin_diff, data=icu_patients_df1)
summary(cox.Albumin_diff)

# fitting a simple Cox model with Cholesterol_max using coxph
cox.Cholesterol_max <- coxph( Surv(Days, Status) ~ Cholesterol_max, data=icu_patients_df1)
summary(cox.Cholesterol_max) # significant

# fitting a simple Cox model with Cholesterol_diff using coxph
cox.Cholesterol_diff <- coxph( Surv(Days, Status) ~ Cholesterol_diff, data=icu_patients_df1)
summary(cox.Cholesterol_diff) # significant
...
# fitting a simple Cox model with Creatinine_max using coxph
cox.Creatinine_max <- coxph( Surv(Days, Status) ~ Creatinine_max, data=icu_patients_df1)
summary(cox.Creatinine_max) # significant

# fitting a simple Cox model with Creatinine_diff using coxph
cox.Creatinine_diff <- coxph( Surv(Days, Status) ~ Creatinine_diff, data=icu_patients_df1)
summary(cox.Creatinine_diff) # significant

# fitting a simple Cox model with Troponin_max using coxph
cox.TroponinI_max <- coxph( Surv(Days, Status) ~ TroponinI_max, data=icu_patients_df1)
summary(cox.TroponinI_max)

# fitting a simple Cox model with Troponin_diff using coxph
cox.TroponinI_diff <- coxph( Surv(Days, Status) ~ TroponinI_diff, data=icu_patients_df1)
summary(cox.TroponinI_diff)

```


```{r}
# fitting series of multivariable Cox proportional hazards models 
cox.mv1 <- coxph(Surv(Days, Status) ~ Age + SOFA + Glucose_max, data=na.omit(icu_patients_df1))
summary(cox.mv1)

cox.mv2 <- coxph(Surv(Days, Status) ~ ICUType + Age + SAPS1 + SOFA + Glucose_max, data=na.omit(icu_patients_df1))
summary(cox.mv2)

cox.mv3 <- coxph(Surv(Days, Status) ~ ICUType + Age + SAPS1 + SOFA + Creatinine_max + MAP_min + Glucose_max, data=na.omit(icu_patients_df1))
summary(cox.mv3)

cox.mv4 <- coxph(Surv(Days, Status) ~ ICUType + Age + SAPS1 + SOFA + Creatinine_max + MAP_min + Albumin_max + Glucose_max + TroponinI_max, data=na.omit(icu_patients_df1))
summary(cox.mv4)
...
# From comparing models with anova() function, we found that cox.mv2 has significant p-value 0.03, thus we choose cox.mv2.
# make further model comparisons to be sure all these variables should be retained using drop1()
cox.mv2 <- coxph(Surv(Days, Status) ~ ICUType + Age + SAPS1 + SOFA + Glucose_max, data=na.omit(icu_patients_df1))
print(drop1(cox.mv2, test="Chisq"))

# remove Age and SAPS1 from the model after checking drop1() result
cox.refit <- coxph(Surv(Days, Status) ~ ICUType + SOFA + Glucose_max, data=na.omit(icu_patients_df1))
