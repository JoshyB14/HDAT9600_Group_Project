...
Regarding which variables to include in the model selection process we decided to first include the basic patient descriptor variables age and gender as it is a good idea to inspect these regardless of the sort of analysis. Furthermore, the type of ICU and length of stay were also included, as were both the SAPS-I score and the SOFA score. We hypothesized the ICU type may be an indicator of the severity of the patient's condition, and thus a reasonable predictor of mortality. The clinical variables are risk assessment measures, ergo they are an obvious choice to include for further inspection. Finally, we included the minimum white blood cell count, ____, ___,  based on their clinical relevance. A low white blood cell count is indicative of a weak immune system, a person with no white blood cells is incapable of fighting infection, and thus is more likely to die from a variety of contitions than someone with a sufficient amount of white blood cells. 
...
```
The clinical variables are SOFA and SAPS-I scores. SOFA stands for sequential organ failure assessment. It is a mortality prediction score based on the degree of dysfunction of six organ systems. The SAPS-I score is another mortality risk prediction measure that is calculated from 14 routine physiological measurements.  
...
ggplot(data = icu_patients_df1, aes(x = SAPS1)) + geom_histogram(fill = "salmon1") 
```
```{r}
...
ggplot(data = icu_patients_df1, aes(x = WBC_min)) + geom_histogram(fill = "green4") 

# WBC mean count
summary(icu_patients_df1$WBC_diff)
ggplot(data = icu_patients_df1, aes(x = WBC_diff)) + geom_histogram(fill = "purple1") 
...
# Univariate fits
age_uni_fit <- glm(in_hospital_death ~ Age, family  = "binomial", data = icu_patients_df1)
gender_uni_fit <- glm(in_hospital_death ~ Gender, family = "binomial", data = icu_patients_df1)
SAPS_uni_fit <- glm(in_hospital_death ~ SAPS1, family = "binomial", data = icu_patients_df1)
SOFA_uni_fit <- glm(in_hospital_death ~ SOFA, family = "binomial", data = icu_patients_df1)
WBCmin_uni_fit <- glm(in_hospital_death ~ WBC_min, family = "binomial", data = icu_patients_df1)
WBCdiff_uni_fit <- glm(in_hospital_death ~ WBC_diff, family = "binomial", data = icu_patients_df1)

# Summaries
summary(age_uni_fit) # significant
summary(gender_uni_fit) # non significant
summary(SAPS_uni_fit) # significant, note SAPS AIC is better than SOFAs, should probably include both
summary(SOFA_uni_fit) # significant
summary(WBCmin_uni_fit) # significant
summary(WBCdiff_uni_fit) # significant, ONLY CHOOSE WBC_MIN OR WBC_DIFF TO AVOID MULTICOLLINEARITY
```
...
cand_1_fit <- glm(in_hospital_death ~ ., family = "binomial", data = icu_patients_df1)
cand_2_fit <- glm(in_hospital_death ~ ., family = "binomial", data = icu_patients_df1)
cand_3_fit <- glm(in_hospital_death ~ ., family = "binomial", data = icu_patients_df1)
...
```{r}
# Skeleton code for diagnostics of the candidate models 

# Residual checks
binnedplot(predict(cand_1_fit), residuals(cand_1_fit))
binnedplot(predict(cand_2_fit), residuals(cand_2_fit))
binnedplot(predict(cand_3_fit), residuals(cand_3_fit))

# Outlier checks
halfnorm(hatvalues(cand_1_fit))
halfnorm(hatvalues(cand_2_fit))
halfnorm(hatvalues(cand_3_fit))
...
# look at the first few lines of icu patients df1
head(icu_patients_df1)

# table the in hospital
