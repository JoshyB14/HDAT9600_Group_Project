...
Regarding which variables to include in the model selection process we decided to first include basic patient descriptor variables: age, gender, and height as it is a good idea to inspect these regardless of the sort of analysis. Furthermore, the type of ICU and length of stay were also included, as were both the SAPS-I score and the SOFA score. The clinical variables are risk assessment measures, ergo they are an obvious choice to include for further inspection. Finally, we included ______ based on their clinical relevance. 
...
summary(SAPS_uni_fit) # significant
