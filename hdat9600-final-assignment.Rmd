...
summary(cox.RespRate_max)  # significant
...
cox.mv3 <- coxph(Surv(Days, Status) ~ SAPS1 + SOFA + WBC_max + HR_max + RespRate_max, data=icu_patients_df1)
summary(cox.mv3)

cox.mv4 <- coxph(Surv(Days, Status) ~ SAPS1 + SOFA + WBC_max + HR_max + RespRate_max + Glucose_max, data=icu_patients_df1)
summary(cox.mv4)
...
```
_Remove SAPS1 and SOFA from the model after checking drop1() result_
```{r}
cox.refit <- coxph(Surv(Days, Status) ~ WBC_max + HR_max, data=icu_patients_df1)
...
Compared to the `cox.refit` model, `cox.mv2` has a lower AIC value, so we prefer the `cox.mv2` model.
