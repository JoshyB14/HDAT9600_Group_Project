...
print(anova(cox.mv1, cox.mv4))

```
_From comparing models with the anova() function, we found that cox.mv2 has significant p-value 0.03, thus we choose cox.mv2._
_Hence make further model comparisons to be sure all these variables should be retained using drop1()_
```{r}
cox.mv2 <- coxph(Surv(Days, Status) ~ ICUType + Age + SAPS1 + SOFA + Glucose_max, data=na.omit(icu_patients_df1))
print(drop1(cox.mv2, test="Chisq"))
```
_Remove Age and SAPS1 from the model after checking drop1() result_
```{r}
